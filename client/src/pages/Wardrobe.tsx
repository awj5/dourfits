...
function WardrobeStage() {
  ...
  return (
    <div id="wardrobeStage">
      <Avatar {...darcel} />
      <button onClick={submitClick} className="bigButton">
        Submit
        <svg viewBox="0 0 15.84 27.18">
          <path d="M2.25,27.18c-.58,0-1.15-.22-1.59-.66-.88-.88-.88-2.3,0-3.18L10.41,13.59,.66,3.84C-.22,2.96-.22,1.54,.66,.66,1.54-.22,2.96-.22,3.84,.66L15.18,12c.88,.88,.88,2.3,0,3.18L3.84,26.52c-.44,.44-1.02,.66-1.59,.66Z" />
        </svg>
      </button>
      <button onClick={resetClick} className="iconButton" id="stageReset">
        <img src="/assets/img/icon-reset.png" alt="Reset" />
      </button>
      <button
        onClick={downloadClick}
        className="iconButton"
        id="stageDownload"
        style={{ pointerEvents: downloadEnabled ? "auto" : "none" }}
      >
...
function Viewer(props: {
  ...
  setViewerMessage: React.Dispatch<React.SetStateAction<string>>;
}) {
  ...
  const checkItemOwned = (title: string, trait: string): boolean => {
    const checkTrait = userTraits.filter((obj) => {
      return (
        obj.value.toLowerCase() === title.toLowerCase() &&
        (obj.trait_type.toLowerCase() === trait.toLowerCase() ||
          (trait === "Background" && obj.trait_type.toLowerCase() === "backgrounds"))
      ); // Hacky code added to make genesis backgrounds available to DF backgrounds with same name
    });
    ...
    const getNFTs = async (page?: string | undefined) => {
      try {
        ...
        for (let x: number = 0; x < userNFTs.ownedNfts.length; x++) {
          let attributes: Record<"value" | "trait_type", string>[] | undefined =
            userNFTs.ownedNfts[x].rawMetadata?.attributes;
  ...
  return (
    <>
      <div id="viewer" ref={viewerRef} onScroll={viewerScroll} onMouseUp={cancelScroll}>
        {viewerItems.map((item, i) => (
          <ViewerItem
            ...
            itemSFXOver={itemSFXOver}
            itemSFXClick={itemSFXClick}
            category={itemCategory}
            item={item}
            //traitOwned={!item.layer ? true : checkItemOwned(item.title, item.trait ?? "")} // TEMP
            traitOwned={true}
      ...
      <button
        onMouseDown={() => scrollMouseDown("up")}
        onMouseUp={cancelScroll}
        style={{ visibility: scrollUp ? "visible" : "hidden", pointerEvents: scrollUp ? "auto" : "none" }}
        className="iconButton viewerUpDown"
        id="viewerUp"
      >
        <img src="/assets/img/icon-arrow.svg" alt="Up" draggable="false" />
      </button>
      <button
        onMouseDown={() => scrollMouseDown("down")}
        onMouseUp={cancelScroll}
        style={{ visibility: scrollDown ? "visible" : "hidden", pointerEvents: scrollDown ? "auto" : "none" }}
        className="iconButton viewerUpDown"
        id="viewerDown"
      >
        <img src="/assets/img/icon-arrow.svg" alt="Down" draggable="false" />
      </button>
    </>
  );
}

function ViewerMenu(props: {
  ownedOnly: boolean | undefined;
  setOwnedOnly: React.Dispatch<React.SetStateAction<boolean | undefined>>;
}) {
  ...
  return (
    <div id="viewerMenu">
      <button onClick={homeClick} className="iconButton" id="menuHome">
        <img src="/assets/img/icon-home.svg" alt="Home" style={{ paddingBottom: "4px" }} />
      </button>
      <button onClick={ownedToggleClick} className={`iconButton ${props.ownedOnly && "selected"}`} id="menuOwnedToggle">
        <svg viewBox="0 0 127.49 121.25" style={{ paddingBottom: "2px" }}>
          <polygon points="63.74 0 83.44 39.91 127.49 46.31 95.61 77.38 103.14 121.25 63.74 100.53 24.35 121.25 31.87 77.38 0 46.31 44.05 39.91 63.74 0" />
        </svg>
      </button>
      ...
      <select id="menuCategories" onChange={categoryChange} value={category}>
        ...
        {categories.map((category, i) => (
          <option value={category.title.toLowerCase().replace("&", "and").replace(/ /g, "-")} key={i}>
            {category.title}
          </option>
        ))}
      </select>
  ...
  return (
    <CategoryContext.Provider value={{ category, setCategory }}>
      <div id="wardrobeViewer">
        ...
        <p id="viewerMessage" style={{ display: viewerMessage ? "inline" : "" }}>
          {viewerMessage}
        </p>
      </div>
...
function Wardrobe() {
  ...
  const [darcel, setDarcel] = useState<Darcel>(
    localStorage.avatarV2 ? JSON.parse(localStorage.avatarV2) : DefaultDarcel
  );
  ...
  return (
    <DarcelContext.Provider value={{ darcel, setDarcel }}>
      <div
        className="section"
        id="sectionWardrobe"
        style={{ display: !isConnected && !urlParams.get("demo") ? "none" : "" }}
      >
        <WardrobeStage />
        <WardrobeViewer />
      </div>
